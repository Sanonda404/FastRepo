...
export async function createIssuePr(owner: string, repo_name: string, data: IssuePrFormOutput): Promise<number> {
  return api<number>(`/pulls/issues/${owner}/${repo_name}`, { method: "POST", body: data });
}
