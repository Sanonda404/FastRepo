        ...
        <div className="mx-auto w-full max-w-6xl px-6 pt-8">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <h1 className="text-xl font-semibold tracking-tight">
              <span className="text-muted-foreground">{owner} /</span> {repository}
            </h1>
            {repoMeta?.is_private && (
            ...
            <div className="ml-auto flex gap-2">
              <button className={buttonVariants({ variant: "outline", size: "sm" })}><Star className="size-3.5" /> Star</button>
              <button className={buttonVariants({ variant: "outline", size: "sm" })}><GitFork className="size-3.5" /> Fork</button>
            </div>
          </div>
          <nav aria-label="Repository navigation" className="mt-6 flex gap-1 overflow-x-auto">
            {navItems.map(([label, Icon]) => (
              <button
                key={label}
                className={`relative flex shrink-0 items-center gap-2 rounded-t-md px-3 pb-3 pt-2 text-sm ${label === "Code" ? "border-b-2 border-primary font-semibold" : "border-b-2 border-transparent text-muted-foreground hover:text-foreground"}`}
              >
          ...
          <aside data-testid="repo-sidebar" className="flex flex-col gap-4">
            <section aria-label="About this repository" className="rounded-xl bg-card p-5 ring-1 ring-foreground/10">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {repoMeta === null ? "Loading description…" : repoMeta.description || "No description provided."}
              </p>
            </section>
